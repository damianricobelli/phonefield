...
const liveCountrySlots: PhoneField.CountrySlots = {
	trigger: {
		className:
			"inline-flex h-10 cursor-default select-none items-center justify-between gap-2 whitespace-nowrap rounded-xl border border-gray-200 bg-[canvas] pr-2.5 pl-3 text-base text-gray-900 hover:bg-gray-100 focus-visible:outline focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-blue-800 data-[popup-open]:bg-gray-100",
	},
	icon: { className: "flex" },
	popup: {
		className:
			"[--input-container-height:3.5rem] origin-[var(--transform-origin)] min-w-[16rem] max-w-[var(--available-width)] max-h-[24rem] overflow-hidden rounded-xl bg-[canvas] text-gray-900 outline-1 outline-gray-200 shadow-lg shadow-gray-200 transition-[transform,scale,opacity] data-[ending-style]:scale-90 data-[ending-style]:opacity-0 data-[starting-style]:scale-90 data-[starting-style]:opacity-0",
	},
	searchInput: {
		className:
			"m-2 h-10 w-[calc(100%-1rem)] rounded-lg border border-gray-200 pl-3.5 text-base font-normal text-gray-900 focus:outline focus:outline-2 focus:-outline-offset-1 focus:outline-blue-800",
	},
	empty: {
		className:
	...
	list: {
		className:
			"max-h-[min(calc(24rem-var(--input-container-height)),calc(var(--available-height)-var(--input-container-height)))] overflow-y-auto overscroll-contain py-2 scroll-py-2 empty:p-0",
	},
	item: {
		className:
			"grid min-w-[max(16rem,var(--anchor-width))] cursor-default grid-cols-[minmax(0,1fr)_auto] items-center gap-2 py-2.5 pr-4 pl-4 text-base leading-5 outline-none select-none data-[highlighted]:relative data-[highlighted]:z-0 data-[highlighted]:text-gray-50 data-[highlighted]:before:absolute data-[highlighted]:before:inset-x-2 data-[highlighted]:before:inset-y-0 data-[highlighted]:before:z-[-1] data-[highlighted]:before:rounded-lg data-[highlighted]:before:bg-gray-900",
	},
};
	...
	return (
		<article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-lg shadow-slate-200/50 ring-1 ring-slate-200/50 md:p-8">
			...
			<p className="mt-1 text-sm text-slate-600">
				Country slots follow Base UI Combobox part props, with styling applied
